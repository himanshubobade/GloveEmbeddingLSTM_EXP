...
def collate_fn(batch):
    ...
    padded_casing_features = [item['casing_features'] + [[0] * 5] * (max_seq_len - len(item['casing_features'])) for item in batch]
...
class GloveEmbeddedBidirectionalLSTM(nn.Module):
    def _init_(self, config):
        super(GloveEmbeddedBidirectionalLSTM, self)._init_()

        # Create embedding layer with GloVe embeddings
        self.embedding = nn.Embedding.from_pretrained(torch.FloatTensor(embeddings), padding_idx=word2idx['[PAD]'], freeze=True)
        self.lstm = nn.LSTM(config.embedding_dim + 5, config.lstm_hidden, batch_first=True, bidirectional=True)
...
optimizer = optim.AdamW(model.parameters(), lr=GloveEmbeddedBidirectionalLSTMConfig.learning_rate, weight_decay=1e-5)
